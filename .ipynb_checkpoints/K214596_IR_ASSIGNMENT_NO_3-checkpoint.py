...
df = pd.read_csv('TF_IDF.csv',encoding='latin1')
...
# showing the unique document_id in the table
df['Document_ID'].unique()

#dropping document_id and labels
x = df.drop(['Document_ID','labels'],axis=1)
y = df['labels']
y = pd.get_dummies(y)
...
def Preprocessing_for_query(query): # in this we clean the tokens and do stemming also
        ...
        stopWords= pp.read_file_content('Stopword-List.txt') #get a preset stopword list
...
data = pd.read_csv('TF_IDF.csv', encoding='latin1')
X = data.drop('Document_ID', axis=1)
print(X.shape)
...
if 'Document_ID' in df.columns:
    X = df.drop(['Document_ID', 'labels'], axis=1)  # Remove Document_ID column
else:
    X = df.drop(['labels'], axis=1)  # If Document_ID is not present, only remove labels column
...
cluster_class_mapping = df.groupby(['cluster', 'labels'])['Document_ID'].apply(list).reset_index()
